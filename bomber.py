...
def check_intr():
    try:
        ...
        bann_text()
        mesgdcrt.FailureMessage("CHECK YOUR INTERNET CONNECTION AND TRY AGAIN")
        sys.exit(2)
...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/IND-GANG/ABomb/archive/dev.zip"
        dir_name = "TBomb-dev"
    else:
        zip_url = "https://github.com/IND-GANG/ABomb/archive/master.zip"
    ...
    if success:
        mesgdcrt.SuccessMessage("ABomb was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update ABomb.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/IND-GANG/ABomb.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/IND-GANG/ABomb.git HEAD")
    sys.exit()
...
def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/IND-GANG/ABomb/raw/dev/.notify"
        else:
            url = "https://github.com/IND-GANG/ABomb/raw/master/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.WarningMessage(
        "THIS TOOL MADE FOR TESTING YOUR SPAM DETECTOR")
    mesgdcrt.WarningMessage(
        "AND SEND SPAM MSG FOR YOUR FRIENDS")
    mesgdcrt.SuccessMessage("ABomb was created by ADIL")
...
def workernode(mode, cc, target, count, delay, max_threads):
    ...
    clr()
    mesgdcrt.SectionMessage("GEARING UP BOMB")
    mesgdcrt.GeneralMessage(
        "Please stay connected to the internet during bombing")
    ...
    mesgdcrt.WarningMessage(
        "THIS TOOL MADE FOR TESTING YOUR SPAM DETECTOR")
    mesgdcrt.GeneralMessage(
        "AND FOR SEND SPAM MSG TO YOUR FRIENDS")
    print()
    input(mesgdcrt.CommandMessage(
        "[CTRL+Z] to stop & [ENTER] to continue"))
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("ABomb will work only in Python v3")
    sys.exit()
...
description = """ABomb - Your Friendly Spammer Application

ABomb can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

ABomb is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by ADIL !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start ABomb with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start ABomb with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start ABomb with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update ABomb")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current ABomb contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current ABomb version")
